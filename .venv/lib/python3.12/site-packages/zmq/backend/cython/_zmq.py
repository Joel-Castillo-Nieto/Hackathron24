...
@inline
@C.exceptval(-1)
def _check_rc(rc: C.dv_int, error_without_errno: bint = False) -> C.dv_int:
    """internal utility for checking zmq return condition

    and raising the appropriate Exception class
    """
    errno: C.dv_int = _zmq_errno()
    PyErr_CheckSignals()
...
@cfunc
@nogil
def free_python_msg(data: p_void, vhint: p_void) -> C.dv_int:
...
class Frame:
    ...
    def __del__(self):
        ...
        with nogil:
            rc: C.dv_int = zmq_msg_close(address(self.zmq_msg))
        _check_rc(rc)
    ...
    def __getbuffer__(self, buffer: pointer(Py_buffer), flags: C.dv_int):  # noqa: F821
...
class Context:
    ...
    def __init__(self, io_threads: C.dv_int = 1, shadow: size_t = 0):
        self.handle = NULL
        self._pid = 0
        self._shadow = False
    ...
    def set(self, option: C.dv_int, optval):
        """
...
class Socket:
    ...
    def monitor(self, addr, events: C.dv_int = ZMQ_EVENT_ALL):
        """
    ...
    def join(self, group):
        ...
        rc: C.dv_int = zmq_join(self.handle, group)
        _check_rc(rc)
    ...
    def leave(self, group):
        ...
        if not zmq.has('draft'):
            raise RuntimeError("libzmq must be built with draft support")
        rc: C.dv_int = zmq_leave(self.handle, group)
        _check_rc(rc)
...
@cfunc
@inline
def _recv_frame(handle: p_void, flags: C.dv_int = 0, track: bint = False) -> Frame:
...
@cfunc
@inline
def _send_frame(handle: p_void, msg: Frame, flags: C.dv_int = 0):
...
@cfunc
@inline
def _send_copy(handle: p_void, buf, flags: C.dv_int = 0):
...
@cfunc
@inline
def _getsockopt(handle: p_void, option: C.dv_int, optval: p_void, sz: pointer(size_t)):
...
@cfunc
@inline
def _setsockopt(handle: p_void, option: C.dv_int, optval: p_void, sz: size_t):
...
def strerror(errno: C.dv_int) -> str:
    """
...
# polling
def zmq_poll(sockets, timeout: C.dv_int = -1):
...
def device(device_type: C.dv_int, frontend: Socket, backend: Socket = None):
    """
...
def _mq_inline(
    ...
    rc: C.dv_int = zmq_msg_init(address(msg))
